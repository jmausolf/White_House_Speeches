...
def WHT3(url):
    ...
    try:
        ...
        print len(clean_paragraph)
        if len(clean_paragraph) == 0:
            #paragraph = content.get_text()
            #print paragraph
            #paragraph = ["".join(x.findAll(text=True)) for x in content.findAll('<div class="legacy-para">')]
            #paragraph_body = "\n\n%s" % ("\n\n".join(paragraph))
            paragraph_body1 = content.get_text()
            paragraph_body2 = ""
